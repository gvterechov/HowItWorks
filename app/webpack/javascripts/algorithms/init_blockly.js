...
$(function() {
  ...
  {
    localStorage.blockly_xml_backup = xml
  }
  // localStorage
  function load_xml()
  {
    let str = localStorage.blockly_xml_backup
    return str || null;
  }
    ...
    function clean_condition_values(values_str) {  // => Array<!String>
      ...
      while (values_str) {
        ...
        for (const values_group of [TRUTH_ALIASES, FALSE_ALIASES]) {
          let prefix = whichprefix(values_str, values_group)
          if (prefix !== null) {
  ...
  function setup_blockly_workspace(workspace) {
    ...
    function saveWorkspaceOnChange(event) {
      ...
      if (event.isUiEvent)
        return;  // игнорируем неизменяющие события

      // подождать немного, пока соберутся все сопутствующие события, потом сохранить
      if (saveWorkspace_timer !== null)
        clearTimeout(saveWorkspace_timer);
      saveWorkspace_timer = setTimeout(() => {
        saveWorkspace_timer = null;
        saveWorkspaceOnChange(null);
      }, 500);
    }
    workspace.addChangeListener(saveWorkspaceOnChange);
  }


  function patch_localization() {
    // find out what lang is ON
    const en = $('#lang').val() == 'en';
    const ru = $('#lang').val() == 'ru';

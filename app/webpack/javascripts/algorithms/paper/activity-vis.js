...
export function redraw_activity_diagram(algorithm_json, trace_json, options) {
	...
	if (options.viewport) {
		let rect = new paper.Rectangle(options.viewport);
		rect.y += 50;
		let topCenter_of_diagram = alg.bbox().topCenter;
		alg.rebase(topCenter_of_diagram, rect.topCenter);
	} else {
		alg.rebase(new paper.Point(options.position || [400, 50]));
	}
